...
from src.layers.nv_encoder import NVEncoder
from src.tools.tools import l2_norm
from src.layers.contrast import Contrast
from src.layers.dv_encoder import DVEncoder
...
class GHCDTI(nn.Module):
    def __init__(self, hid_dim, args, keys, mps_len_dict: dict, attn_drop, feat_dim: dict):
        ...
        self.scencoder = NVEncoder(hid_dim, keys)
        self.scencoder2 = NVEncoder(hid_dim, keys)
        self.mpencoder = nn.ModuleDict({k: DVEncoder(v, hid_dim, attn_drop) for k, v in mps_len_dict.items()})
        self.mpencoder2 = nn.ModuleDict({k: DVEncoder(v, hid_dim, attn_drop) for k, v in mps_len_dict.items()})
        self.constrast = Contrast(hid_dim, args.tau, keys)
        self.distmult = DistMult(self.dim_embedding)
        self.reset_parameters()

    def reset_parameters(self):
        for m in GHCDTI.modules(self):
    ...
    def forward(self, drug_drug, drug_chemical, drug_disease, drug_sideeffect, protein_protein,
        ...
        mloss, dti_re = self.distmult(drug_vector, disease_vector, sideeffect_vector, protein_vector,
                                      ...
                                      protein_sequence, protein_disease, drug_protein, drug_protein_mask)
        L2_loss = 0.
        for name, param in GHCDTI.named_parameters(self):
            if 'bias' not in name:
