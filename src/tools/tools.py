...
# def load_feature_default():
#     drug_feats = torch.from_numpy(
#         numpy.loadtxt("../../data1/feature/rep_sim1_drug.txt")).to(torch.float32).to(device)
#     protein_feats = torch.from_numpy(
#         numpy.loadtxt("../../data1/feature/rep_sim1_protein.txt")).to(torch.float32).to(device)
#
#     with torch.no_grad():
#         drug_linear = nn.Linear(drug_feats.shape[1], 128).to(device)
#         protein_linear = nn.Linear(protein_feats.shape[1], 128).to(device)
#
#         nn.init.xavier_uniform_(drug_linear.weight)
#         nn.init.xavier_uniform_(protein_linear.weight)
#
#         drug_feats = drug_linear(drug_feats)
#         protein_feats = protein_linear(protein_feats)
#
#     disease_feats = torch.randn(440, 128).to(device)
#     sideeffect_feats = torch.randn(12904, 128).to(device)
#
#     node_features = {
#         drug: drug_feats,
#         protein: protein_feats,
#         disease: disease_feats,
#         sideeffect: sideeffect_feats
#     }
#     feat_dim = {
#         drug: 128,
